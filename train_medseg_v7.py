# train_medseg_v5.py (Uses decoder_v3 - PixelLM style)
...
from segment_model.medseg_v7 import build_llm_seg 
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            with autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                outputs_mask = F.interpolate(                      # updated new code
                ...
                if is_main_process() and step < 3:
                    ...
                    if torch.isnan(outputs_mask).any():
                        print(" ⚠️ outputs_mask has NaN values!")
                    if torch.isinf(outputs_mask).any():
                        print(" ⚠️ outputs_mask has Inf values!")
                    else:
                ...
                outputs_mask = torch.nan_to_num(outputs_mask, nan=0.0, posinf=10.0, neginf=-10.0) # updated code
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                if is_main_process():
                    print(f"⚠️ NaN/Inf detected at step {step+1}, "
                          f"loss={loss.item()}, "
...
# # Updated code to load weights
    ...
    ckpt_dir = "/scratch/laks/PRS-Med/weights_medseg/decoder_v4/llm_seg_1"
    ...
    # # 5. (Optional) Load tokenizer if needed
